...
import { resolveLabel, APPAREL_SIZE_GUIDES, APPAREL_SIZE_GUIDE_LABEL } from "../config/brand-pages.js";

/**
 * brand-catalog.js — the shared server-rendered brand catalog page.
 *
 * Backs BOTH /crocs and /adidas. These were previously two ~1250-line
 * files that a diff showed to be ~84% identical, with the entire markup
 * block byte-for-byte the same. Everything that genuinely differed — H1,
 * intro copy, filter chips, size charts, FAQ, SEO strings — is data and
 * now lives in config/brand-pages.js.
 *
 * This module is derived from the former adidas.js rather than crocs.js
 * because Adidas was the SUPERSET: it carried apparel sizing (per gender
 * and top/bottom) that the Crocs page never had. Building from the Crocs
 * version silently dropped that feature — caught by comparing rendered
 * output before and after, not by the test suite.
 *
 * Why consolidating mattered: test/xss-regression.test.js asserts the same
 * escaping rules three times because the vulnerable code existed in three
 * copies. Any fix here previously had to be applied three times, and the
 * escaping bug documented in lib/html.js is exactly what happens when one
 * copy is missed.
 *
 * Adding a brand page is now a config entry, not a new file.
 */
 
export function renderBrandCatalogPage(brandConfig, appConfig, data = {}, locale = "ru", whatsappNumber = "", nonce = null) {
  ...
  // Declared before uiJson, which serialises these for the client script.
  const sizeGuideLabel = resolveLabel(
    { labelKey: BRAND.sizeGuide.labelKey, label: BRAND.sizeGuide.label }, tr, locale
  );
  const kidsSizeGuideLabel = BRAND.kidsSizeGuide
    ? resolveLabel({ labelKey: BRAND.kidsSizeGuide.labelKey, label: BRAND.kidsSizeGuide.label }, tr, locale)
    : sizeGuideLabel;
  const uiJson = JSON.stringify({
    ...
    crocsSizeGuide: sizeGuideLabel,
    crocsKidsSizeGuide: kidsSizeGuideLabel,
    hit: tr.t("hit"),
  ...
  // Per-brand filter chips (Crocs: kids/Jibbitz, Adidas: sneakers/apparel).
  const ALL_TAGS = BRAND.tags.map(t => ({ key: t.key, label: resolveLabel(t, tr, locale) }));
  ...
  /* ── Size charts (shape comes entirely from config) ─────────────── */
  const sizeGuide = BRAND.sizeGuide.rows;
  const kidsSizeGuide = BRAND.kidsSizeGuide ? BRAND.kidsSizeGuide.rows : [];
  const sizeColumns = BRAND.sizeGuide.columns;
  const kidsColumns = BRAND.kidsSizeGuide ? BRAND.kidsSizeGuide.columns : sizeColumns;


  const SizeGuideTable = () => `
    <table class="sg-table" aria-label="${escapeHtml(sizeGuideLabel)}">
      <thead>
        <tr>
          ${sizeColumns.map((c, i) => `<th>${escapeHtml(i === 0 ? BRAND.sizeColumnLabel : c)}</th>`).join("")}
        </tr>
      </thead>
      <tbody>
        ${sizeGuide.map(row => `
          <tr>
            ${row.map((cell, i) => `<td>${escapeHtml(i === row.length - 1 ? cell + " cm" : cell)}</td>`).join("")}
          </tr>`).join("")}
      </tbody>
    </table>`;
  ...
  const productsJson = JSON.stringify(products.map(p => ({
    ...
    isApparel: Array.isArray(p.tags) && p.tags.includes("apparel"),
    // Drives the kids size chart on brands that have one (Crocs). Sent as
    // a precomputed flag rather than the raw tag list, mirroring isApparel.
    isKids: Array.isArray(p.tags) && p.tags.includes("kids"),
    gender: p.gender === "men" ? "men" : "women",
    apparelType: p.apparelType === "bottom" ? "bottom" : "top",
  })));

  /* Apparel size charts (gender × garment type), shared via config. */
  const apparelSizeGuides = BRAND.hasApparel ? APPAREL_SIZE_GUIDES : null;
  const apparelSizeGuideLabel = APPAREL_SIZE_GUIDE_LABEL[locale] ?? APPAREL_SIZE_GUIDE_LABEL.en;
 
  const breadcrumbs = breadcrumbJsonLd(appConfig, [
    { name: tr.t("home"), path: "/" },
    { name: tr.t(BRAND.breadcrumbTitleKey), path: BRAND.path }
  ]);
  const itemList = itemListJsonLd(appConfig, products, BRAND.path);
  const seoNoindex = Boolean(q) || page > 1;
  const seoCanonicalPath = tag && !seoNoindex
    ? `${BRAND.path}?tag=${encodeURIComponent(tag)}`
    : BRAND.path;
  // Derived from the same source as the chips so brand-specific tags
  // (e.g. Adidas "sneakers") get a proper SEO title.
  const tagLabelMap = Object.fromEntries(ALL_TAGS.filter(t => t.key).map(t => [t.key, t.label]));
  const seoTitle = tag && tagLabelMap[tag]
    ? `${tagLabelMap[tag]} — ${tr.t(BRAND.seoTitleKey)}`
    : tr.t(BRAND.seoTitleKey);

  /* FAQ content — targets long-tail "купить adidas originals" delivery/sizing/order
  ...
  // Keys follow <prefix>Q<n>/<prefix>A<n> as declared in config.
  const faqItems = Array.from({ length: BRAND.faqCount }, (_, i) => ({
    question: tr.t(`${BRAND.faqKeyPrefix}Q${i + 1}`),
    answer:   tr.t(`${BRAND.faqKeyPrefix}A${i + 1}`)
  }));
...
<head>
  ...
  ${renderSeoHead({
    appConfig,
    title: seoTitle,
    description: tr.t(BRAND.seoDescriptionKey),
...
${navbarStyles}
    ...
    /* Was .crocs-intro / .adidas-intro — identical rules under two names. */
    .brand-intro {
      max-width: 760px;
      margin: 0 0 28px;
...
<div class="page">
  ...
  ${renderNavbar(appConfig, tr, langSwitcher, waNumber, BRAND.navKey, BRAND.path, q)}
  ...
  <main>
    <div class="inner">
      ...
      <div class="cat-header">
        <h1 class="cat-title">${tr.t(BRAND.h1Key)}</h1>
        <p class="cat-meta">${pluralItems(total, tr)}</p>
      </div>

      <p class="brand-intro ${BRAND.introClass}">${tr.t(BRAND.introKey)}</p>
      ...
      ${showFaq ? `
      <section class="faq-section" aria-label="${tr.t(BRAND.faqTitleKey)}">
        <h2>${tr.t(BRAND.faqTitleKey)}</h2>
        ${faqItems.map(item => `
...
/* Column headers travel with the data so one implementation serves every
   brand (Crocs US/EU/CM, Adidas US/UK/EU/CM). The old crocs.js client
   hardcoded a 4-column header for 3-column adult rows, rendering a stray
   empty <th>. */
const SIZE_GUIDE_COLS = ${JSON.stringify(sizeColumns.map((c, i) => i === 0 ? BRAND.sizeColumnLabel : c))};
const SIZE_GUIDE_KIDS_COLS = ${JSON.stringify(kidsColumns.map((c, i) => i === 0 ? BRAND.sizeColumnLabel : c))};
const HAS_KIDS_GUIDE = ${BRAND.kidsSizeGuide ? "true" : "false"};
const SIZE_GUIDE_APPAREL = ${JSON.stringify(apparelSizeGuides)};
const SIZE_GUIDE_APPAREL_LABEL = ${JSON.stringify(apparelSizeGuideLabel)};
...
function buildSizeGuideHtml(isKids) {
  /* Generic over column count; kids chart used only when the brand has one. */
  const useKids = isKids && HAS_KIDS_GUIDE;
  const guide = useKids ? SIZE_GUIDE_KIDS : SIZE_GUIDE;
  const cols  = useKids ? SIZE_GUIDE_KIDS_COLS : SIZE_GUIDE_COLS;
  const label = useKids ? UI.crocsKidsSizeGuide : UI.crocsSizeGuide;
  const rows = guide.map(function (row) {
    return '<tr>' + row.map(function (cell, i) {
      return '<td>' + clientEscHtml(i === row.length - 1 ? cell + ' cm' : cell) + '</td>';
    }).join('') + '</tr>';
  }).join('');
  return '<table class="sg-table" aria-label="' + clientEscHtml(label) + '">' +
    '<thead><tr>' + cols.map(function (c) { return '<th>' + clientEscHtml(c) + '</th>'; }).join('') + '</tr></thead>' +
    '<tbody>' + rows + '</tbody></table>';
}

function buildApparelSizeGuideHtml(gender, apparelType) {
  if (!SIZE_GUIDE_APPAREL) return '';
  const key = (gender === 'men' ? 'men' : 'women') + '_' + (apparelType === 'bottom' ? 'bottom' : 'top');
  const guide = SIZE_GUIDE_APPAREL[key];
  if (!guide) return '';
  const cols = LOCALE === 'ru' ? guide.cols : guide.colsEn;
  const rows = guide.rows.map(function (r) {
    return '<tr><td>' + clientEscHtml(r[0]) + '</td>' +
      r.slice(1).map(function (v) { return '<td>' + clientEscHtml(v + ' cm') + '</td>'; }).join('') + '</tr>';
  }).join('');
  return '<table class="sg-table" aria-label="' + clientEscHtml(SIZE_GUIDE_APPAREL_LABEL) + '">' +
    '<thead><tr>' + cols.map(function (h) { return '<th>' + clientEscHtml(h) + '</th>'; }).join('') + '</tr></thead>' +
    '<tbody>' + rows + '</tbody></table>';
}
...
function renderQuickView(p, data) {
  ...
  /* ── Sizes HTML ── */
  const isApparel = !!p.isApparel;
  /* Chart choice is per-product: apparel wins, then the kids chart for
     brands that have one, else the default footwear chart. Previously this
     was computed once at module scope, which worked only because the
     Adidas page had no kids chart to switch to. */
  const isKids = !!p.isKids;
  const activeSizeGuideHtml = isApparel
    ? buildApparelSizeGuideHtml(p.gender, p.apparelType)
    : buildSizeGuideHtml(isKids);
  const activeSizeGuideTitle = isApparel
    ? SIZE_GUIDE_APPAREL_LABEL
    : ((isKids && HAS_KIDS_GUIDE) ? UI.crocsKidsSizeGuide : UI.crocsSizeGuide);
  const sizesHtml = variants.length
    ? '<div class="size-section">' +
        ...
        '<div class="sg-panel" id="sg-panel">' +
          '<p class="sg-title">' + activeSizeGuideTitle + '</p>' +
          activeSizeGuideHtml +
        '</div>' +
      '</div>'
    : '';
