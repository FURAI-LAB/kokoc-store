...
export function renderCatalogPage(appConfig, data = {}, locale = "ru", whatsappNumber = "", nonce = null) {
  ...
  const productsJson = JSON.stringify(products.map(p => ({
    slug: p.slug,
    title: p.title,
    // description is sanitized rich-text HTML (p/h3/strong/em/ul/li) —
    // the quick-view modal shows a short plain-text preview, not the
    // formatted version, so strip tags here rather than in client JS.
...
<head>
  ...
  ${jsonLdScripts(nonce, breadcrumbs, products.length ? itemList : null)}
...
<div class="page">
  ...
  ${renderNavbar(appConfig, tr, langSwitcher, waNumber, brand === "adidas" ? "adidas" : "catalog", "/catalog", q)}
  ...
  <main>
    <div class="inner">
      ...
      <div class="cat-header">
        <h1 class="cat-title">${tr.t("catalogTitle")}</h1>
...
// Keep grid heart fill-state in sync if an item is removed from the wishlist drawer
...
function renderQuickView(p, data) {
  ...
  infoEl.querySelector("#qv-add-btn")?.addEventListener("click", () => {
    ...
    window.kokocUpdateCartBadge();
    window.kokocTriggerCartAnim();
