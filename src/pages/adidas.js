...
export function renderAdidasPage(appConfig, data = {}, locale = "ru", whatsappNumber = "", nonce = null) {
  ...
  const uiJson = JSON.stringify({
    ...
    crocsSizeGuide: locale === "ru" ? "Таблица размеров Adidas" : "Adidas Size Guide",
    hit: tr.t("hit"),
  ...
  const ALL_TAGS = [
    { key: "",           label: tr.t("all") },
    { key: "sneakers",   label: locale === "ru" ? "Кроссовки" : "Sneakers" },
    { key: "ballet",     label: locale === "ru" ? "Балетки" : "Ballet flats" },
    { key: "apparel",    label: locale === "ru" ? "Одежда" : "Apparel" },
  ...
  /* ── Size guide table data (Adidas Originals official chart) ───── */
  const sizeGuide = [
    ["US 4",    "UK 3.5",  "36",    "22.1"],
  ...
  const sizeGuideLabel = locale === "ru" ? "Таблица размеров Adidas" : "Adidas Size Guide";
  ...
  /* Adidas Asia official apparel size charts — gender × garment type. */
  ...
  const apparelSizeGuideLabel = locale === "ru" ? "Таблица размеров Adidas (одежда)" : "Adidas Apparel Size Guide";
 
  const breadcrumbs = breadcrumbJsonLd(appConfig, [
    { name: tr.t("home"), path: "/" },
    { name: tr.t("adidasTitle"), path: "/adidas" }
  ]);
  const itemList = itemListJsonLd(appConfig, products, "/adidas");
  const seoNoindex = Boolean(q) || page > 1;
  const seoCanonicalPath = tag && !seoNoindex
    ? `/adidas?tag=${encodeURIComponent(tag)}`
    : "/adidas";
  const tagLabelMap = { new: tr.t("new"), hit: tr.t("hit"), limited: tr.t("limited"), sale: tr.t("sale") };
  const seoTitle = tag && tagLabelMap[tag]
    ? `${tagLabelMap[tag]} — ${tr.t("adidasSeoTitle")}`
    : tr.t("adidasSeoTitle");
  ...
  const faqItems = [
    { question: tr.t("adidasFaqQ1"), answer: tr.t("adidasFaqA1") },
    { question: tr.t("adidasFaqQ2"), answer: tr.t("adidasFaqA2") },
    { question: tr.t("adidasFaqQ3"), answer: tr.t("adidasFaqA3") },
    { question: tr.t("adidasFaqQ4"), answer: tr.t("adidasFaqA4") }
  ];
...
<head>
  ...
  ${renderSeoHead({
    appConfig,
    title: seoTitle,
    description: tr.t("adidasSeoDescription"),
...
<div class="page">
  ...
  ${renderNavbar(appConfig, tr, langSwitcher, waNumber, "adidas", "/adidas", q)}
  ...
  <main>
    <div class="inner">
      ...
      <div class="cat-header">
        <h1 class="cat-title">${tr.t("adidasPageH1")}</h1>
        <p class="cat-meta">${pluralItems(total, tr)}</p>
      </div>

      <p class="adidas-intro">${tr.t("adidasIntro")}</p>
      ...
      ${showFaq ? `
      <section class="faq-section" aria-label="${tr.t("adidasFaqTitle")}">
        <h2>${tr.t("adidasFaqTitle")}</h2>
        ${faqItems.map(item => `
...
function buildSizeGuideHtml() {
  const rows = SIZE_GUIDE.map(([us, uk, eu, cm]) =>
...
function buildApparelSizeGuideHtml(gender, apparelType) {
  ...
  const rows = guide.rows.map(r =>
    '<tr><td>' + r[0] + '</td>' + r.slice(1).map(v => '<td>' + v + ' cm</td>').join('') + '</tr>'
  ).join('');
  return '<table class="sg-table" aria-label="' + SIZE_GUIDE_APPAREL_LABEL + '">' +
...
function renderQuickView(p, data) {
  ...
  const activeSizeGuideHtml = isApparel ? buildApparelSizeGuideHtml(p.gender, p.apparelType) : sizeGuideHtml;
  const activeSizeGuideTitle = isApparel ? SIZE_GUIDE_APPAREL_LABEL : UI.crocsSizeGuide;
